...
class LoginForm(forms.Form):
    username=forms.CharField(label='用户名',
                             ...
                             attrs={'class':'form-control','placeholder':'请输入用户名'})) #required是否为空
    password=forms.CharField(label='密码',widget=forms.PasswordInput(
                             attrs={'class':'form-control','placeholder':'请输入密码'}))
    captcha=CaptchaField(label='验证码')
    def clean(self):#数据的验证
...
class RgForm(forms.Form):
    username=forms.CharField(
        ...
        min_length=3,
        required=True,
        widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入用户名'})) #required是否为空
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(RgForm, self).__init__(*args, **kwargs)
...
class ChangePasswordForm(forms.Form):#修改密码框
    ...
    def __init__(self, *args, **kwargs):#传入一个user
        if 'user' in kwargs:
            self.user = kwargs.pop('user')
        super(ChangePasswordForm, self).__init__(*args, **kwargs)#这样就有了user可使用了
    def clean(self):
...
class ForgotPasswordForm(forms.Form):
    email=forms.EmailField(
        ...
        min_length=3,
        required=True,
        widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入绑定过的邮箱'})) #required是否为空
    ...
    def clean_email(self):#验证用户名
        email=self.cleaned_data['email'].strip()
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError('邮箱不存在')
        return email
    def clean_verification_code(self):#验证验证码
        verification_code = self.cleaned_data.get('verification_code', '').strip()
        if verification_code == '':
            raise forms.ValidationError('验证码不能为空')
            # 判断验证码
